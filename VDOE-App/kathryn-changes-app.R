...
# load data ---------------------------------------------------------------
virginia <- st_read("va_acs_simple_3/va_2019_simple_3.shp")
superintendent <- st_read("super_simple/superintendent_simple.shp")
vccs <- st_read("vccs/vccs.shp")
hs <- st_read("hs/hs.shp")
data_download <- read.csv("data_download_2_reldif.csv")
data_download$DIS12 <- formatC(round(as.numeric(as.character(data_download$DIS12 )), 0))
df <- readRDS("hi_ed_acs.rds")  
hi_ed <- df[c(1,2,9,14:54)]

colleges<- st_read("colleges/colleges.shp")

demo_df <- read_excel("Demo.xlsx", col_names = TRUE)

pell <- readRDS("pell_acs.rds")
...
virginia[virginia$NAMELSA == "COLONIAL HEIGHTS CITY", "CO_Per"] <- 100

pal <- colorBin(palette ="Blues", domain = virginia$CO_Per, bins = 7, na.color = "transparent")

m = leaflet(data = superintendent, options = leafletOptions(minZoom = 7, maxZoom = 10)) %>% 
  ...
  addPolygons(data = virginia, color = "#5A5766", opacity = 1, weight = 1, fillOpacity = 0, label = ~ virginia$NAMELSA)%>%
...
ui <- fluidPage(
  ...
  fluidRow(width = 12,
           ...
           column(10, align = 'center', 
              em('This dashboard was created at the Social and Decision Analytics Division of the Biocomplexity Institute and Initiative, University of Virginia.')),
           column(1)),
    ...
    tabPanel(h4("Virginia High School Map"), 
             ...
             fluidRow(width = 12,
                      ...
                      column(4, p(tags$span(' Orange ', style = "background-color: orange; color: white;border-radius: 25px; white-space: pre-wrap;"), 
                               ...
                               #   The size of dot is relative to the percentage of Virginia 18-24 year olds residing in a given county.
                                  #p(tags$span(' Green ', style = "background-color: #0F5257; color: white;border-radius: 25px; white-space: pre-wrap;"), 
                                  #'circles locate community colleges; click on the circle for a link to the community college website.'),  
                                  p('Thick borders indicate Superintendent Districts.'),
                                p(em('Please be patient as the map loads below.'))#,
                               # p(),
                                #checkboxInput('check1','Show Community Colleges', value = TRUE)
                             ),
                      column(1)
                      ),
...
server <- function(input, output, session) {
  ...
  output$data_downloads <- DT::renderDataTable({
    DT::datatable(data_download[,-c(1,5, 30, 29)] ,
 ...
 # display <- reactive({
  #  if (input$check1 ){
   #   m  %>% addCircleMarkers(data = vccs, radius = 7, fillColor = "#0F5257", fillOpacity = 0.5, stroke = TRUE, color = "#0F5257",opacity = .7,  weight =1,  popup = label2)
    #} else{
     # m
    #}
  #})
  ...
  output$FT_PT <- renderPlot({
    ...
    ggplot(selected_type, aes(y = as.factor(Description), x = `In-State Percentage`, fill = Description)) +
      ...
      labs(title = paste("Enrollment Status", "\n", type_title)) +
